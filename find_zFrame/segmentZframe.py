def segment_zFrame(in_img, img_type='MR', withPlots=False):
    ...
    hist_y, hist_x=np.histogram(in_img_np.flatten(), bins=int(in_img_np.max()/8))
    hist_x = hist_x[:-1]
    ...
    if withPlots: print(hist_diff_zc)
...
def segment_zFrame_filesystem(in_file, out_file, img_type):
    import SimpleITK as sitk
    sitk.WriteImage(segment_zFrame(sitk.ReadImage(in_file), img_type), out_file)
